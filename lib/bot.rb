...
class Bot
  ...
  attr_reader :channels, :info

  def initialize
    @info = Bot.get('https://slack.com/api/users.list')["members"].select do
      |v| v['name'] == 'rubycapstoneproject'
    end[0]
  ...
  def chat_post_message(channel, text, time_stamp)
    result = Bot.post('https://slack.com/api/chat.postMessage', body: {channel: channel, text: text, thread_ts: time_stamp})
    puts result
  end
  ...
  def conversation_answers(channel, time_stamp)
    ...
    answers['messages'][0]['reply_users'].include? @info['id'] if ! answers['messages'][0]['reply_users'].nil?
  end
